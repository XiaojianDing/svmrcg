function [y,y1,y2]=svmvalrand(x,xsup,w,b,kernel,kerneloption,n3,span,framematrix,vector,dual)
...
if ~strcmp(kernel,'frame') | nargin<9;
    framematrix=[];
    vector=[];
end;
if nargin <11
...
elseif isfield(xsup,'datafile') | isfield(x,'datafile');  % data is stored in file and not in memory
    ...
    for ch1=1:chunks1
        ...
        for ch2=1:chunks2
            ...
            kchunk=svmkernelrand(x1,kernel,kerneloption,x2);
            %kchunk=svmkernel(x(ind2,:),kernel,kerneloption,xsup(ind1,:));
  ...
  % ps=svmkernelrand(x,kernel,kerneloption,xsup,framematrix,vector,dual);
